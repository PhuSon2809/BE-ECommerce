...
function Dialog({ open, setOpen, children, className }: DialogProps) {
  return (
    <Transition show={open}>
      <DialogTailwind className='relative z-[200]' onClose={setOpen}>
        <TransitionChild
        ...
        <div className='fixed inset-0 z-10 w-screen overflow-y-auto'>
          <div className='flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0'>
            ...
            >
              <DialogPanel
                className={`relative transform overflow-hidden rounded-lg bg-white transition-all min-w-[400px] ${className}`}
              >
                {children}
              </DialogPanel>
            </TransitionChild>
