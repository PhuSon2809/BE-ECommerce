...
function Search({ className, placeholder = 'Search', variant = 'container' }: SearchProps) {
  return (
    <div
      className={`flex items-center gap-2 xs:min-w-full sm:min-w-[320px] ${variant === 'container' ? ' h-12 px-5 bg-greyMain rounded-[44px]' : 'h-11 px-3 bg-white/[.44] rounded-lg border-blackMain/[.1] border border-solid'}  focus-within:bg-white ${className}`}
    >
